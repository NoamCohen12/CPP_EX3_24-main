...
void Road_Building::play_card(Catan& catan, Player& player) {
    ...
    while (idEdge1 < 1 || idEdge1 > 72 || !player.check_edge_valid_during(*(catan.get_board()), idEdge1) || idHex1 <= 0 || idHex1 >= 18) {        cout << "Invalid input:location of your Edge is not valid" << endl;
        cout << "Choose the first Hexagon to build more time: " << endl;
        idHex1 = player.readValidInt();
        cout << "Choose the first Edge to build more time: " << endl;
        idEdge1 = player.readValidInt();
    }
    Edge* temp1 = catan.get_board()->get_edge_new(idEdge1);
    ...
    Edge* temp2 = catan.get_board()->get_edge_new(idEdge2);
